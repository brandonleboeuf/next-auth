  ...
  return (
    ...
    >
      ...
      <header className="flex justify-between w-full px-12 py-8">
        ...
        <div>
          {!session && (
            <>
              <button
                className="flex items-center bg-[#e50914] text-white text-sm px-4 py-2 ml-2 rounded"
                onClick={() => signIn()}
              >
                Sign in
              </button>
            ...
            <>
              <div className="flex items-center text-sm text-white">
                <img
                  ...
                  alt={`${session.user.email}'s image`}
                  className="h-10 max-w-full align-middle border-none rounded-full shadow"
                />
                <button
                  className="flex items-center bg-[#e50914] text-white text-sm px-4 py-2 ml-2 rounded"
                  onClick={() => signOut()}
      ...
      <main className="flex flex-col items-center justify-center flex-1 px-20 text-center">
        <h1 className="max-w-lg text-5xl font-medium tracking-wide text-white">
          {session
        ...
        <div className="flex mt-4">
          ...
          />
          <button
            className="flex items-center bg-[#e50914] ml-0.5 text-white text-xl px-8"
            onClick={() => signIn()}
          >
            Get Started
            <svg
